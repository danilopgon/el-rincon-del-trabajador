...
export default function Personalizacion() {
  ...
  function switchTab(next: Tab) {
    ...
    requestAnimationFrame(() => {
      ...
      gsap.fromTo(items, { y: 16, opacity: 0 }, { y: 0, opacity: 1, duration: 0.35, stagger: 0.06, ease: 'power2.out' });
    });
  }
          ...
          <div className="pers__gallery" ref={galleryRef}>
            {slots.map((s, i) => (
              <div key={`${tab}-${i}`} style={{ position: 'relative' }}>
                ...
                <div style={{ width: '100%', height: '100%', background: 'var(--color-cream-deep)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
